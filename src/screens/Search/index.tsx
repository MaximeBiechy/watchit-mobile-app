...
const providers = [
  {
    name: 'Netflix',
    logo: assets.images.WatchProviders.netflix2,
  },
  {
    name: 'Prime',
    logo: assets.images.WatchProviders.prime2,
  },
  {
    name: 'Disney',
    logo: assets.images.WatchProviders.disneyPlus2,
  },
  {
    name: 'Max',
    logo: assets.images.WatchProviders.max2,
  },
  {
    name: 'Canal',
    logo: assets.images.WatchProviders.canal2,
  },
  {
    name: 'Paramount',
    logo: assets.images.WatchProviders.paramount2,
  },
  {
    name: 'Apple',
    logo: assets.images.WatchProviders.apple2,
  },
];
    ...
    <FlatList
      ListHeaderComponent={
        <>
          ...
          <CastListComponent
            cast={results.results.filter((item) => item.mediaType === 'actor' && item.name && item.profilePath)}
            imageWidthRatio={5}
          />
          {results.results.length === 0 && <Text style={styles.sectionTitle}>{t('suggestions')}</Text>}
          {results.results.some(
            (item) => item.mediaType === 'movie' && item.title && item.posterPath && item.releaseDate && item.voteAverage,
